...
function __txtcmdLoader() {
  ...
  */
  function splitOutsideTextcmds(text, splitregex) {
    let segs = []
    let lastLoc = 0
    let splitter = new RegExp(splitregex)
      ...
      apply.substrings(source,
        (head, body, matches) => { // inside textcmd; head and body are strings
            ...
            if(cmd !== undefined && 'argDelimiter' in cmd)
              parsedBody = splitOutsideTextcmds(body, cmd.argDelimiter)
            ...
            ret.push({type: 'inside', head: head, body: parsedBody, matches: matches})
...
// }
  ...
  */
  function renderElement(elem, cmdsetArg = undefined) {
    ...
    elem.innerHTML = ''
    elem.appendChild(parsePreprocessEval(source, cmdset))
    elem.classList.add('tc-rendered')
  }
