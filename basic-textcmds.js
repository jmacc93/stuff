...
function __basicCmdsetLoader(){
  ...
  */
  ...
  basicCmdset.link = {
    evaluate: (arg) => {
      let linkurl = getArgNumber(arg, 0, 'string')
      let elem    = getArgNumber(arg, 1, 'element')
      ...
      return elementThen('a', linkelem => {
        if(linkurl[0] === '.') // relative link
          linkelem.href = linkurl.slice(1)
        else if(linkurl.startsWith('http')) // absolute link w/ http:// given
        ...
        tabs.appendChild(elementThen('div', bar => {
          ...
          for(const item of items) {
            bar.appendChild(elementThen('span', tabbutton => {
              ...
              tabbutton.appendChild(getArgNumber(item, 0, 'element'))
  ...
  */
  ...
  basicCmdset.code = {
    evaluate: (arg) => {
      let language = getArgNumber(arg, 0, 'string').trim()
      let source   = getArgNumber(arg, 1, 'string').trim()
